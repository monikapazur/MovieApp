...
class ToWatchMovieAdapter(val context: Context, private var listener: OnToWatchMovieItemLongClick) :
    RecyclerView.Adapter<ToWatchMovieAdapter.MovieViewHolder>() {

    private val moviesList = ArrayList<MovieDetails>()

    fun setToWatchMovies(list: List<MovieDetails>) {
        moviesList.clear()
        moviesList.addAll(list)
        notifyDataSetChanged()
    }

    fun deleteToWatchMovie(movie: MovieDetails, position: Int) {
    ...
    inner class MovieViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        init {
            view.setOnLongClickListener {
                listener.onToWatchMovieLongClick(moviesList[adapterPosition], adapterPosition)
                true
            }
        }
    }
...
interface OnToWatchMovieItemLongClick {
    fun onToWatchMovieLongClick(movie: MovieDetails, position: Int)
}
