...
class WatchedMovieAdapter(val context: Context, private var listener: OnWatchedMovieItemLongClick) :
    RecyclerView.Adapter<WatchedMovieAdapter.MovieViewHolder>() {

    private val moviesList = ArrayList<MovieDetails>()

    fun setWatchedMovies(list: List<MovieDetails>) {
        moviesList.clear()
        moviesList.addAll(list)
        notifyDataSetChanged()
    }

    fun deleteWatchedMovie(movie: MovieDetails, position: Int) {
    ...
    inner class MovieViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        init {
            view.setOnLongClickListener {
                listener.onWatchedMovieLongClick(moviesList[adapterPosition], adapterPosition)
                true
            }
        }
    }
...
interface OnWatchedMovieItemLongClick {
    fun onWatchedMovieLongClick(movie: MovieDetails, position: Int)
}
